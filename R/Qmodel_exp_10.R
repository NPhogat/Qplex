#'@name Qmodel_exp_10
#'@aliases Qmodel_exp_10
#'@title To develop the statistical linear models and graphs with R-square
#'values and linear model equation.
#'@description Function \code{Qmodel_exp_10} works on the output of the function
#'\code{Repsd}. Function \code{Qmodel_exp_10} is designed for the sandwich assay.
#'It develops the statistical linear fit models of Normalized
#'Intensities vs Concentration, in a range with two values lesser than
#'the complete assay range. Here, it is in the range of 0-10 nM,
#'after removing the 15 nM and 20 nM and their respective intensities.
#'It represents the results in the form of plots, with embedded
#'R-square value,linear model equation and graph title in the plot.
...
#'@details Function \code{Qmodel_exp_10} works on the output of the function
#'\code{Repsd}. Function \code{Qmodel_exp_10} is designed for the sandwich assay.
#'It develops the statistical linear fit models of Normalized
#'Intensities vs Concentration, in a range with two values lesser than
#'the complete assay range. Here, it is in the range of 0-10 nM,
...
#'@keywords Qdata
#'@examples
#'Example is similar like function Qmodel_exp
...
#'plot_sand_r
#'@export
setMethod("Qmodel_exp_10", signature = "Qdata", definition =

            function (Qdata, greenQD = TRUE, graphtitle = "Green QD",
                      W = 0.02, P = 0.02){
              crep_g <- slot(Qdata,"cgData")
              nidata_g <- crep_g[1:(nrow(crep_g)-2),"Green_NI"]

              crep_r <- slot(Qdata,"crData")
              nidata_r <- crep_r[(3:nrow(crep_r)),"Red_NI"]

              if (greenQD == TRUE){
                Concn_g <- slot(Qdata,"Concentration_CRP")
                Concn.new_g <- Concn_g[1:(nrow(crep_g)-2),]
              }

              else if (greenQD == FALSE){
                Concn_r <- slot(Qdata,"Concentration_IL6")
                Concn.new_r <- Concn_r[3:nrow(crep_r),]
              }
              else {
              ...
              sd.ni_g <- sd_g[1:(nrow(sd_g)-2),"Green_NI"]

              sd_r <- slot(Qdata,"sdrData")
              sd.ni_r <- sd_r[3:(nrow(sd_r)),"Red_NI"]
