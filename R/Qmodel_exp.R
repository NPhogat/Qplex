...
#'@description Function \code{Qmodel_exp} works on the output of the function
#'\code{Repsd}. Function \code{Qmodel_exp} is designed for the sandwich assay.
...
#'\code{Repsd}. Function \code{Qmodel} is designed for the sandwich assay.
...
setMethod("Qmodel_exp", signature = "Qdata", definition =
            ...
            function (Qdata, greenQD = TRUE, graphtitle = "Green QD",
              ...
              sd.ni_g <- sd_g[1:(nrow(sd_g)),"Green_NI"]

              sd_r <- slot(Qdata,"sdrData")
              sd.ni_r <- sd_r[1:(nrow(sd_r)),"Red_NI"]
